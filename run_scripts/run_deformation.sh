...
export DATA_DIR="/scratch-ssd/Datasets/deformation/nerf"
...
CUDA_VISIBLE_DEVICES=0 python3 train.py -s $DATA_DIR --port $port --expname "deforming_hemisphere/time5" --configs arguments/mdnerf-dataset/cube.py --lambda_w 2000 \
  --lambda_rigidity $RIGIDITY_LAMBDA --lambda_spring $LAMBDA_SPRING --lambda_momentum $LAMBDA_MOMENTUM --lambda_velocity $LAMBDA_VELOCITY --view_skip 1 --time_skip 20 \
  --k_nearest 20 --lambda_isometric $ISOMETRY --reg_iter 11000 --staticfying_from 10000 --use_wandb --wandb_project "deforming_hemisphere" --wandb_name "init" --white_background True
