...
export DATA_DIR="/scratch-ssd/Datasets/deformgs/synthetic"
...
CUDA_VISIBLE_DEVICES=1 python3 train.py -s "/scratch-ssd/Datasets/deformgs/synthetic/scene_3/" --port $port --expname "synthetic/${SCENE}_overfit" --configs arguments/mdnerf-dataset/cube.py --lambda_w 2000 \
  --lambda_rigidity $RIGIDITY_LAMBDA --lambda_spring $LAMBDA_SPRING --lambda_momentum $LAMBDA_MOMENTUM --lambda_velocity $LAMBDA_VELOCITY --view_skip 1 --time_skip 1 \
  --k_nearest 20 --lambda_isometric $ISOMETRY --reg_iter 11000 --staticfying_from 10000 --use_wandb --wandb_project "synthetic_${SCENE_3}" --wandb_name "init"
