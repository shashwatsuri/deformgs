...
class multipleview_dataset(Dataset):
    def __init__(
        ...
        self.image_paths, self.image_poses, self.image_times, self.view_ids, self.time_ids= self.load_images_path(cam_folder, cam_extrinsics,cam_intrinsics,split)
        if split=="test":
            ...
            for i in range(len(self.image_paths)):
                ...
                cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                                    ...
                                    time = time, mask=None, time_id=None,view_id=None) # default by monocular settings.
    ...
    def load_images_path(self, cam_folder, cam_extrinsics,cam_intrinsics,split):
        ...
        for idx, key in enumerate(cam_extrinsics):
            ...
            view_id = os.path.basename(extr.name)[5:-4]
            images_folder=os.path.join(cam_folder,"cam"+view_id.zfill(2))
            ...
            for i in image_range:    
                time_id=i+1
                image_path=os.path.join(images_folder,"frame_"+str(time_id).zfill(5)+".png")
        ...
        return image_paths, image_poses,image_times, view_ids,time_ids
